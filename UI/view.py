...
class View(ft.UserControl):
    ...
    def load_interface(self):
        ...
        row1=ft.Row([ft.Container(self._btnAnalizzaOggetti, width=250), ft.Container(self._txtIdOggetto,  width=250),ft.Container(self._btnCompConnessa,  width=250) ],
                                          alignment=ft.MainAxisAlignment.CENTER)
        self._page.controls.append(row1)

        #row2
        self._ddLUN=ft.Dropdown(label="LUN", border_color="orange", disabled=True)
        self._btnCerca=ft.ElevatedButton(text="Cerca Oggetti", on_click=self._controller.handleCerca, bgcolor="orange",color="white",width=200, disabled=True)
        row2=ft.Row([ft.Container(width=250), ft.Container(self._ddLUN, width=250),ft.Container(self._btnCerca, width=250),], alignment=ft.MainAxisAlignment.CENTER)
